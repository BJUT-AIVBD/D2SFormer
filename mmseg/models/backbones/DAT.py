...
from mmseg.models.backbones.ECA import ECAB
...
class CSWinBlock(nn.Module):
    ...
    def __init__(self, dim, patches_resolution, num_heads,
        ...
        self.W = None
        ######################32024.3.14
        self.conv_block = ECAB(dim, compress_ratio=compress_ratio, gamma=gamma, b=b)
    ...
    def forward(self, x):
        ...
        img = self.norm1(x)
        ########################2024.3.14
        conv_x = self.conv_block(img.reshape(B, H, W, C).permute(0, 3, 1, 2))
        ...
        ####################2024.4.10
        x = x + self.drop_path(attened_x) + conv_x
...
class DAT(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=64, depth=[1, 2, 21, 1], split_size=7,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, use_chk=False, pretrained=None,
                 compress_ratio=3,gamma=2, b=1):
        super().__init__()
        ...
        self.stage1 = nn.ModuleList([
            CSWinBlock(
                ...
                compress_ratio=compress_ratio, gamma=gamma, b=b)
